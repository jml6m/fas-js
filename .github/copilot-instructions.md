...
- The `lock-files` gate ([`.github/PROTECTED_FILES.json`](PROTECTED_FILES.json)) locks the proven pre-v2 **Locked** set (core algorithm impls, behavioral tests, the guard group + specs, all workflows, build config, `LICENSE`, [`docs/coverage-policy.md`](../docs/coverage-policy.md)) as **exact paths** — so **new files are Open by default** and v2's RegEx/GNFA files won't trip it. The grouped path notation in [`AGENTS.md`](../AGENTS.md) is documentation shorthand only; [`.github/PROTECTED_FILES.json`](PROTECTED_FILES.json) is the canonical exact-path list. **Open** on purpose: the API surface (governed by the api-contract check, not the lock — `src/modules.ts`, `test/helpers/publicApiContract.js`), barrels, demo, dev/build config, `package.json`/`package-lock.json`, and governance docs.
